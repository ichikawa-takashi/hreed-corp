(function () {
  ...
  function runOpeningScene(root, done) {
    ...
    // --- intro: the connecting "e" (IT) sparks to life first, then a -------
    // glowing circuit draws outward from it in both directions; HR and ED
    // materialize along that line as it reaches them, outward from the
    // connector — visualizing IT as the invisible bridge between them.
    ...
    var leftGroup = dotMeshes.concat(bodyMeshes, [wordMeshes[0], wordMeshes[1]]);
    var connectorMesh = wordMeshes[2];
    var rightGroup = [wordMeshes[3], wordMeshes[4]];

    // everything materializes in place (no big scatter) — small, invisible,
    // waiting for the circuit to reach it
    leftGroup.concat(rightGroup).forEach(function (mesh) {
      mesh.material.opacity = 0;
      mesh.scale.setScalar(0.7);
    });
    connectorMesh.material.opacity = 0;
    connectorMesh.scale.setScalar(0.4);
    ...
    var connectorBox = bbox(connectorMesh);
    var connectorCx = (connectorBox.max.x + connectorBox.min.x) / 2;
    var connectorCy = (connectorBox.max.y + connectorBox.min.y) / 2;
    var leftEdgeX = Math.min.apply(
      null,
      leftGroup.map(function (m) { return bbox(m).min.x; })
    );
    var rightEdgeX = Math.max.apply(
      null,
      rightGroup.map(function (m) { return bbox(m).max.x; })
    );
    var leftSpan = connectorCx - leftEdgeX + 30;
    var rightSpan = rightEdgeX - connectorCx + 30;

    // two thin glowing bars, pivoted at the connector so they grow outward
    var lineHeight = 22;
    var leftLineGeo = new THREE.PlaneGeometry(1, lineHeight);
    leftLineGeo.translate(-0.5, 0, 0); // pivot at its right edge
    var rightLineGeo = new THREE.PlaneGeometry(1, lineHeight);
    rightLineGeo.translate(0.5, 0, 0); // pivot at its left edge

    var leftLine = new THREE.Mesh(
      leftLineGeo,
      new THREE.MeshBasicMaterial({ color: 0x0c998a, transparent: true, opacity: 0.85 })
    );
    leftLine.position.set(connectorCx, connectorCy, -14);
    leftLine.scale.set(0.0001, 1, 1);
    logoGroup.add(leftLine);

    var rightLine = new THREE.Mesh(
      rightLineGeo,
      new THREE.MeshBasicMaterial({ color: 0x0c998a, transparent: true, opacity: 0.85 })
    );
    rightLine.position.set(connectorCx, connectorCy, -14);
    rightLine.scale.set(0.0001, 1, 1);
    logoGroup.add(rightLine);
    ...
    tl.to(camera.position, { z: restDistance, duration: 1.6, ease: "power2.out" }, 0)
      // the connector sparks to life first
      .to(connectorMesh.material, { opacity: 1, duration: 0.3, ease: "power1.out" }, 0.3)
      .to(connectorMesh.scale, { x: 1, y: 1, z: 1, duration: 0.4, ease: "back.out(3)" }, 0.3)
      // the circuit draws outward from it, in both directions
      .to(leftLine.scale, { x: leftSpan, duration: 0.75, ease: "power2.out" }, 0.65)
      .to(rightLine.scale, { x: rightSpan, duration: 0.75, ease: "power2.out" }, 0.65)
      // HR materializes along the line, outward from the connector to the icon
      .to(
        leftGroup.map(function (m) { return m.material; }),
        {
          opacity: 1,
          duration: 0.4,
          stagger: { each: 0.07, from: leftGroup.length - 1 },
          ease: "power1.out",
        },
        0.75
      )
      .to(
        leftGroup.map(function (m) { return m.scale; }),
        {
          x: 1,
          y: 1,
          z: 1,
      ...
      // ED materializes along the line, outward from the connector to "d"
      .to(
        rightGroup.map(function (m) { return m.material; }),
        { opacity: 1, duration: 0.4, stagger: 0.08, ease: "power1.out" },
        0.75
      )
      .to(
        rightGroup.map(function (m) { return m.scale; }),
        { x: 1, y: 1, z: 1, duration: 0.4, stagger: 0.08, ease: "back.out(2.2)" },
        0.75
      )
      // the circuit has done its job — fade it out
      .to(
        [leftLine.material, rightLine.material],
