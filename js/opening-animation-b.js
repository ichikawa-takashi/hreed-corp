(function () {
  ...
  function runOpeningScene(root, done) {
    ...
    // --- intro: a relay — HR assembles, a beat of silence, the connecting -
    // "e" (IT) snaps in with a digital glitch, then ED is pulled in right
    // after it, like a baton handed off: 人材 → IT → 教育.
    ...
    var hrMeshes = bodyMeshes.concat([wordMeshes[0], wordMeshes[1]]);
    var connectorMesh = wordMeshes[2];
    var edMeshes = [wordMeshes[3], wordMeshes[4]];

    hrMeshes.forEach(function (mesh) {
    ...
    // the connector stays put and materializes with a glitch
    connectorMesh.material.opacity = 0;

    // ED waits just to the right of the connector, ready to be pulled in
    edMeshes.forEach(function (mesh, i) {
      mesh.position.set(150 + i * 90, -20 + Math.random() * 30, 110);
      mesh.rotation.set(0, 0.55, 0);
      mesh.material.opacity = 0;
    });
    ...
    tl.to(camera.position, { z: restDistance, duration: 1.6, ease: "power2.out" }, 0)
      // beat 1: HR assembles
      .to(
        hrMeshes.map(function (m) { return m.position; }),
        { x: 0, y: 0, z: 0, duration: 1.1, stagger: 0.06, ease: "expo.out" },
        0.1
      )
      .to(
        hrMeshes.map(function (m) { return m.rotation; }),
        { x: 0, y: 0, z: 0, duration: 1.1, stagger: 0.06, ease: "expo.out" },
        0.1
      )
      .to(
        hrMeshes.map(function (m) { return m.material; }),
      ...
      // a beat of quiet, then beat 2: the connector glitches to life
      .add(glitchIn(connectorMesh), 1.75)
      // beat 3: ED is pulled in immediately after, like a baton hand-off
      .to(
        edMeshes.map(function (m) { return m.position; }),
        { x: 0, y: 0, z: 0, duration: 0.4, stagger: 0.08, ease: "back.out(2.2)" },
        2.1
      )
      .to(
        edMeshes.map(function (m) { return m.rotation; }),
        { x: 0, y: 0, z: 0, duration: 0.4, stagger: 0.08, ease: "power3.out" },
        2.1
      )
      .to(
        edMeshes.map(function (m) { return m.material; }),
